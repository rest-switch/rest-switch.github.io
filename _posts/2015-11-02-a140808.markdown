...
Once the A140808 device is able to connect with the web server, check the <code>channels-stats</code> endpoint to see the devices registered with it. The endpoint is currently not secured, so that is something that needs to be moved behind the /secure/ route to be password protected. Issue the following command to see the devices registered with the server:
