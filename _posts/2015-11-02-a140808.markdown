...
One command worth noting is the <code>./rs-docker-util.sh enter</code> command. This command will run a bash shell in the running container allowing ssh-like access to the instance. This is handy for getting inside to review log files or to change configuration. Also notice the <code>setpass</code> command used to configure the Nginx login password without entering the container (more on this later).
...
Finally, in the upper-right corner of the web page, click the "Sign In" menu item, and enter the email address and password assigned previously (see <a href="#bweb-config-pw">setpadd</a> above), and finally click the "Sign In" button.
