...
toc: >
          <ul class="nav nav-stacked">
            ...
            <li><a href="#building-webserver">Building&nbsp;Webserver</a>
              <ul class="nav nav-stacked">
...
<a class="anchor" id="bfirm-hlmrm04"></a>
...
After the build finishes successfully, the firmware file <code>openwrt-ramips-rt305x-hlk-rm04-squashfs-sysupgrade.bin</code> will be placed into the <code>hlk-rm04/bin</code> directory. For details on the sysupgrade.bin file, refer to the <a href="http://wiki.openwrt.org/doc/howto/generic.sysupgrade" target="_blank">OpenwWRT wiki</a>. The sysupgrade file does not include a bootloader, so it can only be used to update an already running A140808 device. In a later section of this article, instructions for doing so are included. In the case of a new device, it is necessary to build a full firmware image which takes a few more steps.
...
Tip: If the password contains any special characters like \ or &amp; the BASH shell will try and intprepert and consume these characters. It is always a good idea to quote protect passwords when passing on the command-line as shown above to prevent this from happening.
...
Nov  2 21:56:19 a140808 daemon.debug a140808[2014]: ws_connect: connect request suceeded
...
Once the A140808 device is able to connect with the webserver, check the <code>channels-stats</code> endpoint to see the devices registered with it. The endpoint is currently not secured, so that is something that needs to be moved behind the /secure/ route to be password protected. Issue the following command to see the devices registered with the server:

{% highlight sh %}
 
$ wget -qO- --no-check-certificate "https://52.1.48.84/channels-stats?id=ALL"
...
Now that the A140808 device has a connection to the webserver, it is now possible to issue a command to it. To issue a command, the web page control panel needs to be configured with the serial number (aka Device ID) of the device to control, in this case <code>akbmkzh80</code>  See <a href="#bfirm-uboot">Adding u-boot</a> above for details on the device serial number. The other configuration task is to enter the email address and password that was set for the signed url hash above (see <a href="#bweb-config-pw">Configure Passwords</a>).

The Device Control Panel web page has two device sections for controling two different unique devices. Paste the serial number in the upper-right edit field where it says "Device ID" of either one.
