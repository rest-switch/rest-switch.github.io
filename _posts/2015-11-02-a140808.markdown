...
Once the certificates are in place, start building the Docker image by typing <code>make</code>. The build process begins by attempting to download the official centos 7 image for Docker. After the image is downloaded, it is automatically updated via a <code>yum update</code> command then Nginx and its modules are built. In this case, the Nginx modules are the wandenberg pushstream module, and a custom sha256-hmac <a href="https://github.com/rest-switch/server_nginx/tree/master/docker/docker_build/ngx_hmac_auth_module" target="_blank">signed url authentication module</a> which is used to authenticate requests from the Android client. The benefit of single-use signed-urls in this application is that they allow for simple one-shot messages to be passed in the clear without all of the <a href="http://www.infoq.com/articles/HTTPS-Connection-Jeff-Moser" target="_blank">setup overhead</a> of an encrypted connection. Considering the latency of the mobile phone connection over a carrier or wifi network, shaving as many round-trips off really adds up quickly and provides for a more responsive user interface. More on this later.
...
To run locally, execute the <code>make run</code> command now and skip ahead to the <a href="#bweb-config-pw">Configure Passwords</a> section below. Otherwise, to deploy to a remote server, the image must be packaged and deployed remotely. To package the image for remote deployment, use the <code>make deploy</code> command. The image deployment process will take a couple of minutes as the Linux filesystem is compressed into a tar archive:
...
The last step is to create a running Docker container from the image. It is at this step where any port mapping gets applied (see <a href="#bweb-docker-ports">Docker Ports</a> above) if so desired. To create the running container, type: <code>./rs-docker-util.sh create</code>
...
The key thing to noice from the <code>./rs-docker-util.sh show</code> output above is that the STATUS column above does not indicate a status of EXITED. A status of exited indicates a failure for Nginx to start. One reason Nginx will refuse to start is if missing or invalid SSL certificates are supplied in the <a href="#bweb-certs">Configuring Certificates</a> step above. In order to make a better assessment of why Nginx is failing to start, it may be necessary to forcibly enter the Docker container and manually excute <code>nginx</code> at the command line. To start the Docker container forcibly, use the <code>./rs-docker-util.sh debug</code> command.

At this point the container is running and it is possible to enter the running container with the <code>./rs-docker-util.sh enter</code> command. Type <code>exit</code> to leave the container when finished browsing around.
...
drwxr-xr-x   5 root root  360 Dec  6 18:32 dev
drwxr-xr-x  48 root root 4096 Dec  6 17:54 etc
...
dr-xr-xr-x 113 root root    0 Dec  6 18:32 proc
dr-xr-x---   3 root root 4096 Dec  6 18:32 root
drwxr-xr-x   5 root root 4096 Dec  6 18:32 run
lrwxrwxrwx   1 root root    8 Aug 14 21:00 sbin -> usr/sbin
drwxr-xr-x   2 root root 4096 Jun 10  2014 srv
dr-xr-xr-x  13 root root    0 Dec  6 18:32 sys
drwxrwxrwt   7 root root 4096 Dec  6 18:34 tmp
drwxr-xr-x  13 root root 4096 Aug 14 21:00 usr
drwxr-xr-x  20 root root 4096 Dec  5 01:51 var
...
At this point we are ready to configure an A140808 device for use with the system and run an end-to-end test! The first thing to do is to set the host name and port of the web server on the A140808 device. Access the device using ssh (see <a href="#booting-ssh">Booting and SSH</a>). Once connected, edit the <code>/etc/config/cloudcfg</code> file to set the host name and port (8443 in this example). The daemon will need to be restarted to pickup the new value.
...
Now that the A140808 device has a connection to the webserver, it is now possible to issue a command to it. To issue a command, the web page control panel needs to be configured with the serial number of the device to control, in this case <code>akbmkzh80</code>  See <a href="#bfirm-uboot">Adding u-boot</a> above for details on the device serial number. The other configuration task is to enter the email address and password that was set for the signed url hash above (see <a href="#bweb-config-pw">Configure Passwords</a>).

The Device Control Panel web page has two device sections for controling two different unique devices. Paste the serial number in the upper-right edit field of either one. In the upper-right corner of the web page, click the "Sign In" menu item, then email address and password for the hash, and finally click the "Sign In" button.

At this point the configuration is complete. Clicking "Button 1" should result in relay 1 clicking shut for 250 ms. Likewise, clicking "Button 2" should result in relay 2 clicking shut for 250 ms. Now that the system is working end-to-end, we can build the Android client next.
