...
The last step is to create a running Docker container from the newly installed image. It is at this step where any port mapping gets applied (see <a href="#bweb-docker-ports">Docker Ports</a> above) if so desired. To create the running container, type: <code>./rs-docker-util.sh create</code>
...
To assign the password used to secure the control panel web page, use the <code>setpass</code> command supplying the email address and password to be used: <code>./rs-docker-util.sh setpass "myuser@domain.com" "mysecret"</code>
...
Finally, in the upper-right corner of the web page, click the "Sign In" menu item, and enter the email address and password for the hash (see <a href="#bweb-config-pw">sethash</a> above), and finally click the "Sign In" button.
