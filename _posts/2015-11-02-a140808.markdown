...
Assuming all went well with the build, the image is ready to be run locally or packaged for deployment on a remote server. To run locally, use the <code>make run</code> command. To package the image for deploying remotely, use the <code>make deploy</code> command. The image deployment process will take a couple of minutes:
...
As indicated in the output message, the deployment file in this case is <code>restswitch_web_docker_8e2dd31572da.tar.xz</code>  This file in conjunction with the <code>rs-docker-util.sh</code> file are the only two files needed to deploy the docker image remotely.

At this point, take a moment to see what all the <code>rs-docker-util.sh</code> script provides. Many of the commands found in the Makefile are duplicated in this script. This is so make is not required on the remote deployment machine. Use <code>./rs-docker-util.sh --help</code> to display the command-line options:
...
One command worth noting is the <code>enter</code> command. Typing <code>./rs-docker-util.sh enter</code> will run a bash shell in the running container. This is handy for getting inside to review log files and to change configuration. Also notice the <code>sethash</code> and <code>setpass</code> commands: these command are used to configure Nginx settings without entering the container (more on this later).

At this point we are ready to create a docker container from the docker image created above. The Nginx instance running within the container uses the standard ports 80 for http and 443 for https. These ports can be mapped to any value by docker when the container is created. These docker port values can be changed by editing the <code>rs-docker-util.sh</code> file. It is worth noting that the ability to create a container also exists in the <code>Makefile</code> via the <code>make start</code> or <code>make run</code> commands, and these values can equally be edited in this file as well.
